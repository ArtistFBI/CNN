...
def detect(save_img=False):
    with open('/Users/anton/Documents/CNN/YOLOv5/conf.json') as json_file:
        conf = json.load(json_file)
    save_dir, source, weights, view_img, save_txt, imgsz = \
        Path(conf['save_dir']), conf['source'], conf['weights'], conf['view_img'], conf['save_txt'], conf['img']
    ...
    # Initialize
    set_logging()
    device = select_device(conf['device'])
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred = model(img, augment=conf['augment'])[0]

        # Apply NMS
        pred = non_max_suppression(pred, conf['conf'], conf['iou_thres'], classes=conf['classes'], agnostic=conf['agnostic_nms'])
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                # Write results
                for *xyxy, conff, cls in reversed(det):
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        line = (cls, *xywh, conff) if conf['save_conf'] else (cls, *xywh)  # label format
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * len(line) + '\n') % line)

                    if save_img or view_img:  # Add bbox to image
                        label = '%s %.2f' % (names[int(cls)], conff)
